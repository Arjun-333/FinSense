...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Calculate Total Spent (Only Type = 'expense' in current month)
        const total = expRes.data
          .filter(
            (item) =>
              item.type === "expense" && new Date(item.date) >= startOfMonth,
          )
          .reduce((acc, curr) => acc + Number(curr.amount), 0);
        setTotalSpent(total);

        // Calculate Total Income (Only in current month)
        const income = expRes.data
          .filter(
            (item) =>
              item.type === "income" && new Date(item.date) >= startOfMonth,
          )
          .reduce((acc, curr) => acc + Number(curr.amount), 0);
  ...
  // Calculate start of current month for chart data
  const now = new Date();
  const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);

  // Chart Data (Only Expenses in current month)
  const categoryData = expenses
    .filter((item) => item.type === "expense" && new Date(item.date) >= startOfMonth)
    .reduce((acc, curr) => {
  ...
  return (
    <div className="pb-24 space-y-6">
        ...
        <div className="flex justify-between items-end">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mt-1">
              {loading ? (
                <span className="animate-pulse bg-slate-200 dark:bg-slate-700 h-8 w-32 block rounded"></span>
              ) : (
      ...
      <div className="grid grid-cols-2 gap-3 px-4 mb-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-xl font-bold text-gray-900 dark:text-white">
            {loading ? (
              <span className="animate-pulse bg-slate-200 dark:bg-slate-700 h-6 w-24 block rounded"></span>
            ) : (
      ...
      <div className="grid grid-cols-4 gap-2 px-4 mb-6">
        <Link
          to="/budgets"
          className="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm active:scale-95 transition-transform"
        >
